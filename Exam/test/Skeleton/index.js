function solve() {
    ...
    const date = document.querySelector('form input[name="lecture-date"');
    const moduleName = document.querySelector('select');


    let state = {};
   
    function elem(type, content, addClass){
    ...
    function add(e){
        e.preventDefault();
        if(lectureName.value === '' || date.value === '' || moduleName.value === 'Select module...'){
            return;
        }

        let newDateValue = date.value.split('-').join('/').replace('T', ' - ');

        const h4 = elem('h4', `${lectureName.value} - ${newDateValue}`);
        const button = elem('button', 'Del', 'red');
        const li = elem('li', undefined, 'flex');
        li.appendChild(h4);
        ...
        if(!state[moduleName.value]){
            ul = elem('ul');
            let h3 = elem('h3', `${moduleName.value.toUpperCase()}-MODULE`);
            module = elem('div', undefined, 'module');
            lis = [];
