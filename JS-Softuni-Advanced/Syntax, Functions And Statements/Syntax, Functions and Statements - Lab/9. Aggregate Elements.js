...
function aggregateEmelents(input){

    console.log(sumElements(input));
    ...
    function sumInverse(input){
        return input.reduce((acc, value) => acc + 1/value, 0);
    }


    function concatNums(input){
        return input.map(String).reduce((acc, value) => acc + value);
    }
}

aggregateEmelents([1, 2, 3]);
